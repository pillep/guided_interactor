describe 'Integration' do
  def build_interactor(&block)
    interactor = Class.new.send(:include, GuidedInteractor)
    interactor.class_eval(&block) if block
    interactor
  end

  def build_organizer(options = {}, &block)
    organizer = Class.new.send(:include, GuidedInteractor::Organizer)
  ...
  let(:context) { GuidedInteractor::Context.new(steps: []) }
